...
configDatabase(){
    ...
    if eval "$mysqlCmd" | grep -q "$adminTblName"; then 
        ...
        wait
        mysql -e "USE ${dbName}; INSERT INTO ${dbPrefix}admins  (username, password, fullname, role, credit, is_active, ctime, utime) VALUES ('${username}', '${hashedPassword}', 'modiriat', 'admin', '0', '1', '${nowTime}','0');"
